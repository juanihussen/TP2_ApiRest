...
public class JornadaLaboralController {
    ...
    @GetMapping
    public ResponseEntity<List<JornadaLaboralDTO>> obtenerJornadas(@RequestParam(required = false) String nroDocumento, @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate fechaDesde, @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate fechaHasta) {
